...
<style>
    ...
    .stApp {{
        background-image: url("{_BG_SVG_URI}"), linear-gradient(160deg, #0c0a24 0%, #1b1745 55%, #241f5e 100%);
...
def section_bar(text: str, hero: bool = False):
    ...
    cls = "section-bar hero" if hero else "section-bar"
...
def prediction_form():
    ...
    if step == 0:
        ...
        with col1:
            ...
            default_region = draft['region'] if draft['region'] in regions else regions[0]
            region_val = st.selectbox(
                t('region'), options=regions, index=regions.index(default_region),
                help=t('region_help'), key='w_region',
            )
            draft['region'] = region_val
        with col2:
            districts = get_districts(region_val, st.session_state.language)
            default_district = draft['district'] if draft['district'] in districts else districts[0]
            district_val = st.selectbox(
                t('district'), options=districts, index=districts.index(default_district),
                help=t('district_help'), key='w_district',
            )
